...
import static constants.ApiConstantsURL.unassignedUsers.ASSIGNED_UNASSIGNED_USER_TO_STUDENT_ROLE;
import static constants.ApiConstantsURL.students.LIST_OF_STUDENTS_END_POINT_URL;
import static io.restassured.RestAssured.given;
import static org.assertj.core.api.Assertions.assertThat;

public class AssignAccountToStudentRoleApiTest extends BaseApiTest {
    ...
    @Test
    public void verifyAssignAccountToStudent() {
        int accountId = registeredUser.getId();
        String accountLastName = registeredUser.getLastName();
        given()
                .spec(login("Admin"))
                .when()
                .post(ASSIGNED_UNASSIGNED_USER_TO_STUDENT_ROLE, accountId)
                .then()
                .assertThat()
                .statusCode(200);

        List<String> studentsUsersLastNames = given()
                .spec(login("Admin"))
                .when()
                .get(LIST_OF_STUDENTS_END_POINT_URL)
                .then()
                .extract().jsonPath().getList("lastName");
        assertThat(studentsUsersLastNames.contains(accountLastName));
    }
}
